...
void i2c2_init(int BRG);                 // initiates I2C3 module to baud rate BRG
void i2c2_start(void);                   //function iniates a start condition on bus
void reset_i2c2_bus(void);               //Resets the I2C bus to Idle
char send_i2c2_byte(int data);           //basic I2C byte send
char i2c2_read(void);                    //function reads data, returns the read data, no ack
char i2c2_read_ack(void);                //function reads data, returns the read data, with ack
char i2c2_read_nack(void);
unsigned char I2C2poll(char addr);                       // function checks if device at addr is on bus
